...
public class patchBoardController {
	...
	public String patchBoardList(Model model, HttpServletRequest request) {
	...
	public String patchBoardView(Model model, HttpServletRequest request) {
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		// 1. 데이터 처리
...
//					System.out.println("게시글 작성자의 id : " + boardCom.get(i).getMember_id() );
	...
	public String patchBoardWrite(Member member, BoardDTO dto, Model model, HttpServletRequest request,
			@RequestParam("img") MultipartFile multipartFile) {
		// 1. 데이터 처리
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		model.addAttribute("pg", session.getAttribute("pg"));
		String patchBoard = "패치 & 정보 게시판";
		dto.setBoard_category(patchBoard);
	...
	@GetMapping("/patchSearch")
	public String patchSearch(HttpServletRequest request, Model model) {
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		String board_category = "패치 & 정보 게시판";
		// 데이터 처리
		...
		// DB 조회
		if (board_category.equals("패치 & 정보 게시판")) {
