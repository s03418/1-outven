...
public class ImageBoardController {
	...
	public String imageBoardView(Model model, HttpServletRequest request) {
		// 1. 데이터 처리
...
//					System.out.println("게시글 작성자의 id : " + boardCom.get(i).getMember_id() );
	...
	public String reporter_news_BoardList(Model model, HttpServletRequest request) {
		...
		// 전체목록 페이징 처리
		int totalP = (totalA + 15) / 16; // 총 페이지수
	...
	public String imageBoardDelete(Model model, HttpServletRequest request) {
		// 1. 데이터 처리
		int board_num = Integer.parseInt(request.getParameter("board_num"));
		int pg = Integer.parseInt(request.getParameter("pg"));
		// db
	...
	public String boardModifyForm(Model model, HttpServletRequest request) {
		...
		model.addAttribute("board", board);
		model.addAttribute("pg", pg);
		model.addAttribute("detail_category", board.getDetail_category());

		// 3. view 처리 파일 선택
		return "/board/imageBoardModifyForm"; // "/board/boardModifyForm.mustache"
	}

	// 수정하기
	@PostMapping("/board/imageBoardModify")
	public String imageBoardModify(BoardDTO dto, Model model, HttpServletRequest request,
			@RequestParam(name = "img1", required = false) String multipartFile1,
		...
		// 3. view 처리 파일 선택
		return "/board/imageBoardModify"; // "board/boardModify.mustache"
	}

//////////////////////////////////////////////////////
	@GetMapping("/ImageSearch")
	public String searchByNameAndKeyword(Model model, HttpServletRequest request) {
		HttpSession session = request.getSession();
		String board_category = "이미지게시판";
		// 데이터 처리
		...
		// DB 조회
		if (board_category.equals("이미지게시판")) {
			if (name.equals("board_title")) {
				list = boardService.searchByBoard_titleAndStartnumAndEndnumAndBoardCategory(keyword, startnum, endnum,
						board_category);
				totalA = boardService.getTitleCount(keyword); // 수정: 리턴된 리스트의 사이즈 사용
			} else if (name.equals("board_content")) {
				list = boardService.searchByBoard_contentAndStartnumAndEndnumAndBoardCategory(keyword, startnum, endnum,
						board_category);
				totalA = boardService.getContentCount(keyword); // 수정: 리턴된 리스트의 사이즈 사용
			} else if (name.equals("nick_name")) {
				list = boardService.searchByNick_nameAndStartnumAndEndnumAndBoardCategory(keyword, startnum, endnum,
						board_category);
				totalA = boardService.getNickNameCount(keyword); // 수정: 리턴된 리스트의 사이즈 사용
			} else if (name.equals("member_id")) {
				list = boardService.searchByMember_idAndStartnumAndEndnumAndBoardCategory(keyword, startnum, endnum,
						board_category);
				totalA = boardService.getMemberIdCount(keyword); // 수정: 리턴된 리스트의 사이즈 사용
			} else if (name.equals("keyword")) {
				list = boardService.searchByKeywordAndStartnumAndEndnumAndBoardCategory(keyword, startnum, endnum,
						board_category);
