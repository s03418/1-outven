...
public class accidentBoardController {
	...
	public String accidentBoardList(Model model, HttpServletRequest request) {
...
//					System.out.println("게시글 작성자의 id : " + boardCom.get(i).getMember_id() );
	...
	public String accidentBoardWrite(Member member, BoardDTO dto, Model model, HttpServletRequest request,
			@RequestParam("img") MultipartFile multipartFile) {
		// 1. 데이터 처리
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		model.addAttribute("pg", session.getAttribute("pg"));
		String accidentBoard = "사건 & 사고 게시판";
		dto.setBoard_category(accidentBoard);
	...
	@GetMapping("/board/accidentBoardModifyForm")
	public String accidentBoardModifyForm(BoardDTO dto, Model model, HttpServletRequest request) {
		// 1. 데이터 처리하기
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		int pg = Integer.parseInt(request.getParameter("pg"));
		...
		return "/board/accidentBoardModifyForm";
	}

	@PostMapping("/board/accidentBoardModify")
	public String accidentBoardModify(BoardDTO dto, Model model, 
		...
		return "/board/accidentBoardModify";
	}

	// 데이터 삭제
	@GetMapping("/board/accidentBoardDelete")
	public String accidentBoardDelete(Model model, HttpServletRequest request) {
		// 1. 데이터 처리
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		int board_num = Integer.parseInt(request.getParameter("board_num"));
	...
	public String newsSearch(HttpServletRequest request, Model model) {
		...
		// DB 조회
		if (board_category.equals("사건 & 사고 게시판")) {
