...
//					System.out.println("게시글 작성자의 id : " + boardCom.get(i).getMember_id() );
	...
	public String tipBoardWrite(Member member, BoardDTO dto, Model model, HttpServletRequest request,
			@RequestParam("img") MultipartFile multipartFile) {
		// 1. 데이터 처리
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		model.addAttribute("pg", session.getAttribute("pg"));
		String tipBoard = "팁 & 공략 게시판";
		dto.setBoard_category(tipBoard);
	...
	public String tipBoardModifyForm(BoardDTO dto, Model model, HttpServletRequest request) {
		// 1. 데이터 처리하기
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		int pg = Integer.parseInt(request.getParameter("pg"));
	...
	public String tipBoardModify(BoardDTO dto, Model model, 
	...
	public String tipBoardDelete(Model model, HttpServletRequest request) {
		// 1. 데이터 처리
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		int board_num = Integer.parseInt(request.getParameter("board_num"));
	...
	@GetMapping("/tipSearch")
	public String tipSearch(HttpServletRequest request, Model model) {
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		String board_category = "팁 & 공략 게시판";
		// 데이터 처리
		...
		// DB 조회
		if (board_category.equals("팁 & 공략 게시판")) {
