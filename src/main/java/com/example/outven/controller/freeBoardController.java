...
//					System.out.println("게시글 작성자의 id : " + boardCom.get(i).getMember_id() );
	...
	public String freeBoardWrite(Member member, BoardDTO dto, Model model, HttpServletRequest request,
			@RequestParam("img") MultipartFile multipartFile) {
		// 1. 데이터 처리
		HttpSession session = request.getSession();
		model.addAttribute("member", session.getAttribute("member"));
		model.addAttribute("pg", session.getAttribute("pg"));
		String freeBoard = "자유 게시판";
		dto.setBoard_category(freeBoard);
	...
	public String freeBoardModify(BoardDTO dto, Model model, 
	...
	public String newsSearch(HttpServletRequest request, Model model) {
		...
		// DB 조회
		if (board_category.equals("자유 게시판")) {
